...
export default function ViewingMode({ review, setEditMode, prodId }) {
	...
	return (
		<div className="col-12 media mt-4 pb-4" style={{ borderBottom: "2px solid gray" }}>
			<div className="media-body row">
				...
				<p className="col-6">{auth?.admin && `Featured: ${featured.toString()}`}</p>
				{(auth?.username === user || auth?.admin) && (
					<div className="col-6 text-right">
						<button style={{ border: "none", background: "none" }} onClick={() => setEditMode(true)}>
							<MdModeEdit size="25px" color="#007bff" />
						</button>
						<button style={{ border: "none", background: "none" }} onClick={deleteReview}>
